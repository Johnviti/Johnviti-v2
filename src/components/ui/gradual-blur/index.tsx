...
const mergeConfigs = (...configs: Array<Partial<BlurConfig>>): BlurConfig =>
  configs.reduce<BlurConfig>(
    (acc, c) => ({ ...acc, ...c }),
    { ...DEFAULT_CONFIG },
  );
...
const useResponsiveDimension = (
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      let next = config[key];
      if (w <= 480 && config[`mobile${capitalized}` as keyof BlurConfig]) {
        next = config[`mobile${capitalized}` as keyof BlurConfig] as string;
      } else if (w <= 768 && config[`tablet${capitalized}` as keyof BlurConfig]) {
        next = config[`tablet${capitalized}` as keyof BlurConfig] as string;
      } else if (w <= 1024 && config[`desktop${capitalized}` as keyof BlurConfig]) {
        next = config[`desktop${capitalized}` as keyof BlurConfig] as string;
      }
      setValue(next);
    };
